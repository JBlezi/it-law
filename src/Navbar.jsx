      ...
      <div className="inline-block text-left w-full xl:mr-8 relative">
        ...
        {isOpen && (
          <div className="max-h-32">
            ...
            <div className="right-0 mt-2 dark:text-light dark:bg-grey bg-light absolute z-20 hidden xl:block border border-black dark:border-light rounded-lg" role="menu" aria-orientation="vertical" aria-labelledby="menu-button" tabIndex="-1">
              <div className="py-1" role="none">
                {categories.map((category, index) => (
                  <Link key={index} to={`/category/${category}`} className={`block text-right px-4 py-2 text-xl md:text-2xl hover:bg-gray-200 dark:hover:bg-gray-900 w-full ${currentCategory === category ? 'font-bold' : ''}`} role="menuitem" tabIndex="-1" id={`menu-item-${index}`} onClick={() => setIsOpen(false)}>
                    {category.toUpperCase()}
                  </Link>
                ))}
              </div>
    ...
    return (
      <div className="fixed inset-0 z-50 bg-light dark:bg-grey dark:text-light p-5 w-full h-screen overflow-y-scroll no-scrollbar">
        <div className="flex justify-between">
          <img src={logo} alt="Biernath Legal Logo" className="h-16 dark:hidden"/>
          <img src={logoLight} alt="Biernath Legal Logo" className="h-16 dark:block hidden"/>
  ...
  return (
    <nav className="p-4 lg:p-8 xl:p-4 text-black">
      <div className="flex items-center justify-between">
        <Link to="/">
          <img src={logo} alt="Biernath Legal Logo" className="h-12 md:h-16 md:w-28 xl:max-w-none dark:hidden" />
          <img src={logoLight} alt="Biernath Legal Logo" className="h-12 md:h-16 md:w-28 xl:max-w-none dark:block hidden" />
        </Link>
        <div className="relative flex items-center justify-center md:w-2/3 md:mx-8">
          <FaSearch className="absolute left-9 md:left-4 lg:left-6 text-lg text-gray-400 dark:text-light" />
          <input
            type="text"
            placeholder={t('navbar.search')}
            value={searchQuery}
            onChange={handleSearchChange}
            className="pl-10 pr-3 py-3 lg:py-4 lg:pl-16 lg:text-lg rounded-lg dark:text-light w-4/5 md:w-full focus:outline-none focus:placeholder:opacity-0 focus:ring focus:ring-main dark:bg-light-grey dark:placeholder-light"
          />
        </div>
